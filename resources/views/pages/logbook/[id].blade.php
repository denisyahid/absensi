...
mount(function ($id){
    $this->id = $id;
    $data = App\Models\Logbook::findOrFail($id);
    $this->name = $data->name;
    $this->foto = $data->foto;
...
updated([
    'foto' => function(){ $this->edit = true;},
    'tanggal' => function(){ Carbon\Carbon::createFromFormat('d-m-Y', $this->tanggal);}
]);
...
$simpan = function () {
    ...
    if($this->edit){ 
        $this->validate(['foto' => 'required|image|max:3024']);      
        $data['foto'] = $this->foto->store('logbook', 'public');
    }

    $log = App\Models\Logbook::find($this->id);
    $log->update($data);
            ...
            <div class="flex justify-center p-3 mt-2">
                ...
                <form wire:submit="simpan" class="w-full max-w-lg ">
                    ...
                    <label for="dropzone-file"
                        ...
                        <input id="dropzone-file" onchange="" wire:model="foto" type="file" class="hidden" />
                    </label>
                    @if ($foto && $edit)
                        <img src="{{ str($foto->temporaryUrl())->replace('/livewire/preview-file/','/storage/tmp/') }}" class="object-cover w-full border-2"
                            alt="">
                            
                    @else
                        <img src="/storage/{{ $foto }}" class="w-full" alt="">
                    @endif
                    @error('foto')
                        <span class="mt-2 text-xs text-red-500 error">{{ $message }}</span>
                    @enderror
